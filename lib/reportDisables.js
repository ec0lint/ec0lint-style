...
/** @typedef {import('ec0lint-css').RangeType} RangeType */
/** @typedef {import('ec0lint-css').DisableReportRange} DisabledRange */
/** @typedef {import('ec0lint-css').LintResult} StylelintResult */
/** @typedef {import('ec0lint-css').ConfigRuleSettings<any, Object>} StylelintConfigRuleSettings */
...
module.exports = function (results) {
	for (const result of results) {
		...
		const rangeData = result._postcssResult.ec0lintCss.disabledRanges;

		if (!rangeData) continue;

		const config = result._postcssResult.ec0lintCss.config;
