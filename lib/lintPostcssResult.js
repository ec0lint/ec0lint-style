...
function lintPostcssResult(stylelintOptions, postcssResult, config) {
	postcssResult.ec0lintCss.ruleSeverities = {};
	postcssResult.ec0lintCss.customMessages = {};
	postcssResult.ec0lintCss.ruleMetadata = {};
	postcssResult.ec0lintCss.stylelintError = false;
	postcssResult.ec0lintCss.quiet = config.quiet;
	postcssResult.ec0lintCss.config = config;
	...
	if (!isFileFixCompatible) {
		postcssResult.ec0lintCss.disableWritingFix = true;
	}
	...
	for (const ruleName of ruleNames) {
		...
		if (disableFix) {
			postcssResult.ec0lintCss.ruleDisableFix = true;
		}

		postcssResult.ec0lintCss.ruleSeverities[ruleName] =
			(secondaryOptions && secondaryOptions.severity) || defaultSeverity;
		postcssResult.ec0lintCss.customMessages[ruleName] =
			secondaryOptions && secondaryOptions.message;
		postcssResult.ec0lintCss.ruleMetadata[ruleName] = ruleFunction.meta || {};
				...
				postcssRoots.map((postcssRoot) =>
					ruleFunction(primaryOption, secondaryOptions, {
						fix:
							...
							isFileFixCompatible &&
							!postcssResult.ec0lintCss.disabledRanges[ruleName],
						newline,
...
/**
 * There are currently some bugs in the autofixer of ec0lintCss.
 * The autofixer does not yet adhere to ec0lint-css-disable comments, so if there are disabled
 * ranges we can not autofix this document. More info in issue #2643.
 *
 * @param {PostcssResult} postcssResult
 * @returns {boolean}
 */
function isFixCompatible({ ec0lintCss }) {
	// Check for issue #2643
	if (ec0lintCss.disabledRanges.all.length) return false;
