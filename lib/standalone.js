...
const debug = require('debug')('ec0lint-css:standalone');
...
/** @typedef {import('ec0lint-css').LinterOptions} LinterOptions */
/** @typedef {import('ec0lint-css').LinterResult} LinterResult */
/** @typedef {import('ec0lint-css').LintResult} StylelintResult */
/** @typedef {import('ec0lint-css').Formatter} Formatter */
/** @typedef {import('ec0lint-css').FormatterType} FormatterType */
...
async function standalone({
	...
	if ((!files && !isValidCode) || (files && (code || isValidCode))) {
		return Promise.reject(
			new Error('You must pass ec0lint-css a `files` glob or a `code` string, though not both'),
		);
	}
	...
	if (!files) {
		...
		try {
			const postcssResult = await ec0lintCss._lintSource({
				code,
				codeFilename: absoluteCodeFilename,
			});

			stylelintResult = await ec0lintCss._createStylelintResult(
				postcssResult,
				absoluteCodeFilename,
			);
		} catch (error) {
			stylelintResult = await handleError(ec0lintCss, error);
		}
		...
		if (
			fix &&
			postcssResult &&
			!postcssResult.ec0lintCss.ignored &&
			!postcssResult.ec0lintCss.ruleDisableFix
		) {
			returnValue.output =
				!postcssResult.ec0lintCss.disableWritingFix && postcssResult.opts
	...
	if (filePaths.length) {
		...
		const getStylelintResults = absoluteFilePaths.map(async (absoluteFilepath) => {
			...
			try {
				...
				if (postcssResult.ec0lintCss.stylelintError && useCache) {
				...
				if (
					...
					!postcssResult.ec0lintCss.ignored &&
					fix &&
					!postcssResult.ec0lintCss.disableWritingFix
				) {
				...
				return ec0lintCss._createStylelintResult(postcssResult, absoluteFilepath);
			} catch (error) {
				// On any error, we should not cache the lint result
				fileCache.removeEntry(absoluteFilepath);

				return handleError(ec0lintCss, error, absoluteFilepath);
			}
		});
 ...
 */
function handleError(ec0lintCss, error, filePath = undefined) {
	if (error.name === 'CssSyntaxError') {
		return createStylelintResult(ec0lintCss, undefined, filePath, error);
	}

	throw error;
}

module.exports = /** @type {typeof import('ec0lint-css').lint} */ (standalone);
