...
/** @typedef {import('ec0lint-css').ConfigPlugins} StylelintConfigPlugins */
/** @typedef {import('ec0lint-css').ConfigProcessor} StylelintConfigProcessor */
/** @typedef {import('ec0lint-css').ConfigProcessors} StylelintConfigProcessors */
/** @typedef {import('ec0lint-css').ConfigRules} StylelintConfigRules */
/** @typedef {import('ec0lint-css').ConfigOverride} StylelintConfigOverride */
...
/** @typedef {import('ec0lint-css').CodeProcessor} StylelintCodeProcessor */
/** @typedef {import('ec0lint-css').ResultProcessor} StylelintResultProcessor */
...
async function augmentConfigBasic(
	...
	if (allowOverrides) {
		augmentedConfig = addOptions(ec0lintCss, augmentedConfig);
	}
 ...
 */
async function augmentConfigFull(ec0lintCss, filePath, cosmiconfigResult) {
	if (!cosmiconfigResult) {
		return null;
	}
	...
	const configDir = ec0lintCss._options.configBasedir || path.dirname(filepath || '');
 ...
 */
async function extendConfig(ec0lintCss, config, configDir, rootConfigDir, filePath) {
	...
	for (const extendLookup of normalizedExtends) {
		const extendResult = await loadExtendedConfig(ec0lintCss, configDir, extendLookup);
 ...
 */
function loadExtendedConfig(ec0lintCss, configDir, extendLookup) {
	const extendPath = getModulePath(configDir, extendLookup, ec0lintCss._options.cwd);

	return ec0lintCss._extendExplorer.load(extendPath);
}
	...
	for (const pluginLookup of normalizedPlugins) {
		...
		for (const pluginRuleDefinition of normalizedPluginImport) {
			if (!pluginRuleDefinition.ruleName) {
				throw configurationError(
					`ec0lint-css requires plugins to expose a ruleName. The plugin "${pluginLookup}" is not doing this, so will not work with ec0lint-css. Please file an issue with the plugin.`,
				);
			}

			if (!pluginRuleDefinition.ruleName.includes('/')) {
				throw configurationError(
					`ec0lint-css requires plugin rules to be namespaced, i.e. only \`plugin-namespace/plugin-rule-name\` plugin rule names are supported. The plugin rule "${pluginRuleDefinition.ruleName}" does not do this, so will not work. Please file an issue with the plugin.`,
				);
			}
 ...
 * @returns {StylelintConfig}
 */
function addOptions(ec0lintCss, config) {
	const augmentedConfig = {
		...config,
	};

	if (ec0lintCss._options.ignoreDisables) {
		augmentedConfig.ignoreDisables = ec0lintCss._options.ignoreDisables;
	}

	if (ec0lintCss._options.quiet) {
		augmentedConfig.quiet = ec0lintCss._options.quiet;
	}

	if (ec0lintCss._options.reportNeedlessDisables) {
		augmentedConfig.reportNeedlessDisables = ec0lintCss._options.reportNeedlessDisables;
	}

	if (ec0lintCss._options.reportInvalidScopeDisables) {
		augmentedConfig.reportInvalidScopeDisables = ec0lintCss._options.reportInvalidScopeDisables;
	}

	if (ec0lintCss._options.reportDescriptionlessDisables) {
		augmentedConfig.reportDescriptionlessDisables =
			ec0lintCss._options.reportDescriptionlessDisables;
	}

	if (ec0lintCss._options.customSyntax) {
		augmentedConfig.customSyntax = ec0lintCss._options.customSyntax;
	}
