...
const COMMAND_PREFIX = 'ec0lint-style-';
...
/** @typedef {import('ec0lint-style').DisabledRangeObject} DisabledRangeObject */
/** @typedef {import('ec0lint-style').DisabledRange} DisabledRange */
...
module.exports = function assignDisabledRanges(root, result) {
	...
	result.ec0lintStyle.disabledRanges = disabledRanges;
	...
	function getCommandRules(command, fullText) {
		const rules = fullText
			...
			.split(/\s-{2,}\s/u)[0] // Allow for description (f.e. /* ec0lint-style-disable a, b -- Description */).
			.trim()
			.split(',')
			.filter(Boolean)
			.map((r) => r.trim());
