...
const COMMAND_PREFIX = 'ec0lint-css-';
...
/** @typedef {import('ec0lint-css').DisabledRangeObject} DisabledRangeObject */
/** @typedef {import('ec0lint-css').DisabledRange} DisabledRange */
...
module.exports = function assignDisabledRanges(root, result) {
	...
	result.ec0lintCss.disabledRanges = disabledRanges;
	...
	function getCommandRules(command, fullText) {
		const rules = fullText
			...
			.split(/\s-{2,}\s/u)[0] // Allow for description (f.e. /* ec0lint-css-disable a, b -- Description */).
			.trim()
			.split(',')
			.filter(Boolean)
			.map((r) => r.trim());
